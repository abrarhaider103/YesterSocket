    ...
    if (startIO !== false) {
        ...
        io.on('connection', (socket) => {
            ...
            function disconnect() {
                ...
                if (sessionID && profile_rooms[sessionID]) {
                profile_rooms[sessionID] = profile_rooms[sessionID].filter(u => u.id !== socket.id);
            ...
            socket.on("join", (userData) => {
                ...
                const existingUserIndex = profile_rooms[sessionID].findIndex(u => u.email === email);

                if (existingUserIndex !== -1) {
                    // Update existing user's socket ID (for reconnection)
              ...
              socket.on("kickUser", ({ userId, sessionID }) => {
                const kicker = profile_rooms[sessionID]?.find(u => u.id === socket.id);
                if (!kicker || !kicker.isAdmin) return;
            
                const userToKick = profile_rooms[sessionID]?.find(u => u.id === userId);
                if (userToKick) {
                  profile_rooms[sessionID] = profile_rooms[sessionID].filter(u => u.id !== userId);
